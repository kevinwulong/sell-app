...
<script>
  ...
  export default {
    data() {
        return {
            zonlyContent: this.onlyContent,
            zselectType: this.selectType
        }
    },
     props: {
	    ratings: {
     ...
     methods: {
     	...
     	toggleContent(event){
     		...
     		this.zonlyContent =!this.zonlyContent
     		this.$emit('increment', 'onlyContent', this.zonlyContent)
     	},
            needShow(type, text) {
                if (this.zonlyContent && !text) {
                  return false
                }
                if (this.zselectType ===ALL) {
                  return true
                }
                else {
                  return type === this.zselectType
                }
            }
     },
     computed: {
